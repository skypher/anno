...
43. **AI military production removed** — the former `RequestMilitary` path inferred an army from scalar gold, population, personality, difficulty, and a local cooldown, then spawned a hand-authored unit mix beside the first Kontor. That behavior now waits for decoded player-slot production and command state. Scenario-authored units, player commands, combat, and movement remain live. Regressions `military_ai_does_not_synthesize_unit_request` and `ai_does_not_synthesize_military_units` cover controller and dispatcher behavior.
44. **AI building placement removed** — `AiController` no longer turns an implementation-defined goods priority list, treasury, population, personality, difficulty, or Kontor position into a building order. The former dispatcher chose a definition, fertility island, footprint, cost, material requirements, and construction time from local heuristics. Scenario-authored and player-requested construction remain live; new AI construction waits for decoded player-slot commands. Regressions `economic_ai_does_not_synthesize_build_request` and `ai_does_not_synthesize_buildings_from_priority_list` pin the controller and simulation paths.
...
47. **AI auto trade routes removed** — `AiController` no longer creates a route request from warehouse count, gold, difficulty, or cooldown state. The former dispatcher generated an all-goods route, an unassigned ship, and a fixed 1000-gold cost from those heuristics; live AI trade now waits for decoded scenario/AI route state. Existing source-authored trade routes retain their normal execution path. Regression `ai_does_not_generate_trade_route_without_source_route_state` keeps an eligible AI from synthesizing a route, ship, or cost.
...
51. **AI diplomacy reactions removed** — `Simulation::tick_diplomacy` no longer creates or clears diplomatic relations from a scalar strength score, spawns warehouse defenders, assigns escorts, or buys escort warships from `Diplomacy::War`. Source AI diplomacy, defense, and naval orders wait for decoded player-slot state. Sim tests include `ai_score_does_not_create_war`, `ai_score_does_not_end_war`, `economic_ai_does_not_declare_war`, `ai_score_does_not_dispatch_offensive_raid`, `ai_does_not_synthesize_defenders_when_threatened`, and `ai_does_not_synthesize_escort_warship_when_at_war`
...
62. **AI warehouse-defense synthesis removed** — `tick_diplomacy` no longer turns a nearby hostile unit, a warehouse location, and a 100-gold local cost into infantry defenders. Scenario-authored units and player commands remain live; AI defense waits for decoded source orders. Regressions `ai_does_not_synthesize_defenders_when_threatened` and `ai_does_not_defend_against_neutral` cover war and neutral proximity.
...
68. **AI gold reinvestment threshold removed** — the former `AiController::tick` shortcut that halved build, military, and trade cooldowns whenever AI gold exceeded 15 000 has been removed. Those retained snapshot fields no longer drive live AI behavior until the original player-slot pacing fields are decoded. Sim regressions cover rich and poor AI state behavior.
...
70. **AI naval-escort synthesis removed** — `MilitaryUnit` retains `escort_ship: i32` (-1 = none), and `combat::tick_escort_targets` continues to follow an explicitly present link and clear it after its trade ship becomes inactive. `tick_diplomacy` no longer assigns idle warships or buys a `SmallWarship` from war state and a 500-gold local cost. Source-authored escort links remain executable; new AI escort assignments wait for decoded source orders. Regressions cover non-synthesis, target tracking, and inactive-ship cleanup.
...
73. **Fixed building-combat damage removed** — the former live military scheduler turned a default 100-health pool, a two-tile adjacency radius, and 5 damage per hostile unit into building destruction, walkability changes, and tile-clear events. Those values had no decoded source ownership, so hostile units no longer damage or remove buildings through that local formula. `BuildingInstance::health` remains for modeled disaster effects, while the tile-clear renderer bridge remains available to source-derived destruction events. Regressions `war_does_not_synthesize_building_destruction` and `buildings_safe_from_neutral_units` cover the removed scheduler.
...
98. **Combat damage floating numbers removed** — the former `DamageEvent` and `floating_nums` overlay were audited out of the live game surface. Combat remains free of modern floating damage labels.
...
107. **Diversified AI military spawn removed** — the former 40/30/20/10 unit mix and its per-unit prices were implementation-only behavior attached to `RequestMilitary`. New AI military units now wait for decoded source state rather than a local spawn distribution.
...
136. **No scalar-score AI relation flips** — `tick_diplomacy` no longer declares war, sues for peace, or backs out of war from the implementation-only `civilization_power` ratio. It also does not fabricate wartime defenders or escorts; relation changes and tactical AI commands require explicit/source-loaded diplomacy paths until the original `FUN_0042adf0` / `FUN_0042b000` / `FUN_0042b160` state-machine and cooldown fields are ported. Regressions prove an overpowered military AI stays Neutral without a source relation change, and an outmatched AI at War does not auto-reset to Neutral from the score alone.
...
140. **No scalar-score AI offensive raids** — `tick_diplomacy` no longer sends idle AI units at an enemy warehouse because a `civilization_power`/score comparison says the AI is winning. It also no longer fabricates wartime defenders or naval escorts from existing `Diplomacy::War`; source AI commands wait for decoded player-slot state and cooldowns. Regression `ai_score_does_not_dispatch_offensive_raid` locks the removed offensive path down.
...
144. **No gold-stockpile AI cooldown acceleration** — `AiController::tick` no longer treats a 15 000-gold treasury as permission to halve retained build, military, or trade cooldown fields. That threshold had no decoded source owner and no longer changes live AI pacing. Regression `rich_ai_does_not_accelerate_cooldowns_from_gold_stockpile` pins the removed path.
145. **No source-free AI route synthesis** — active AI controllers no longer infer a trade route from owning two Kontors, a gold threshold, difficulty, and an implementation-defined cooldown. The removed path created an all-goods `TradeRoute`, small trader, and fixed 1000-gold debit without a decoded route or ship command record. Source-authored routes remain executable; new AI routes wait for extracted state. Regression `ai_does_not_generate_trade_route_without_source_route_state` covers the formerly eligible case.
146. **No synthetic SHIP4 warship behavior** — scenario-loaded owner-5 warships no longer receive an implementation-defined 16-tile square patrol or a forced player-0 war relation merely because a static ship exists. SHIP4 still supplies the hull, owner, name, and spawn coordinate; movement and diplomacy now retain the scenario state until decoded per-ship route/command and relationship semantics say otherwise. Regression `ship4_native_warship_does_not_receive_synthetic_patrol` covers a PIRAT hull at the former injection point.
147. **No source-free AI military synthesis** — no active AI personality now converts difficulty, gold, population, a warehouse position, or a cooldown into newly spawned units. The removed path also created a fixed composition, prices, maintenance, and spawn offsets without a decoded production order. Regression coverage pins both the controller and live simulation cases.
148. **No source-free AI construction synthesis** — no active AI personality now generates `RequestBuild` actions from a locally authored priority ladder or places the selected building through cost, fertility, variety, and nearby-tile heuristics. The retained action shape is inert pending decoded source AI orders. Regression coverage pins controller and live simulation behavior.
149. **No source-free AI tax or liquidation changes** — `AiController` no longer changes tax rates from local satisfaction/gold thresholds or sells warehouse goods above a locally chosen reserve when gold is low. The retained action shapes are inert pending decoded source commands; direct player tax and trade commands remain active. Regression `ai_does_not_synthesize_tax_change_from_satisfaction` covers the former satisfaction trigger.
150. **No source-free AI wartime reinforcements** — `tick_diplomacy` no longer turns nearby hostile units into spawned defenders, or war state plus a trade ship into an escort assignment or a purchased warship. Existing source-loaded military units and explicit escort links remain active. Regressions `ai_does_not_synthesize_defenders_when_threatened` and `ai_does_not_synthesize_escort_warship_when_at_war` cover both removed paths.
151. **No fixed combat building destruction** — live military ticks no longer infer building damage and destruction from a fixed health pool, radius, and per-unit damage constant. Existing source-derived destruction rendering utilities and disaster state remain separate; direct combat destruction waits for decoded source damage and targeting behavior. Regression `war_does_not_synthesize_building_destruction` pins the formerly destructive wartime setup.
152. **No inferred tower-defense damage** — `Kanon` values remain decoded from `haeuser.cod`, but live towers and castles no longer derive a range of `4 + cannons` and damage of `0.05 × cannons` from that count. Tower targeting, timing, range, and damage wait for decoded combat state. Regression `tower_defense_does_not_infer_range_or_damage` pins the source-waiting behavior.
153. **No legacy ten-second combat resolver** — `FUN_00451890` is called from the main dispatcher on every engine update and advances per-entity state-machine accumulators before dispatching individual handlers. The former `Simulation::tick_military` instead invoked a local nearest-enemy scan, fixed range/timer table, direct health subtraction, and unit removal every ten seconds. That live substitution is removed; source-authored units and stored command state remain while combat waits for extracted state handlers. `TIMING_SPECS` now marks the retained ten-second military timer as a stand-in rather than binary-verified combat cadence. Regression `military_tick_does_not_synthesize_legacy_combat` pins the removed behavior.
154. **No legacy tile-step military movement** — `FUN_00451890` advances record-local velocity and state accumulators before dispatching figure handlers. The former entity tick instead converted an implementation-defined `move_speed` table into one-tile movement and substituted local ocean/A* routing. Player targets, patrol links, and authored positions remain stored, but live military units no longer move through that replacement path until source figure-state movement is extracted. Regression `entity_tick_does_not_synthesize_legacy_unit_movement` pins the removed behavior.
