...
pub struct IslandMap {
    ...
    /// Root local cell returned by `FUN_00463250` for each static map cell.
    /// A kind-`0x34` descriptor may name any cell of an oriented footprint,
    /// while source target tests resolve that cell back to the footprint root.
    source_land_target_roots: Vec<Option<(i32, i32)>>,
    ...
    /// The island's crop fertilities, copied from
    /// [`anno_formats::szs::Island::fertilities`] — i.e. the crop bits
    /// of the `INSEL5[0x5C]` bitmask at runtime `island+0x5c`, one bit
    /// index per slot with 7 padding unused slots. Use
    /// `active_fertilities()` to iterate the typed values.
    ///
    /// These are *not* `INSEL5[0x0C..0x14]`; that array is the
    /// per-settlement owner slots (`FUN_00468740`,
    /// `1602_exe.c:72888-72897`), where 7 means "unsettled".
...
impl IslandMap {
    ...
    pub fn from_island(island: &Island, cod_buildings: &[CodBuilding]) -> Self {
        ...
        let mut source_land_target_roots = vec![None; size];
                ...
                .find(|def| def.source_id == tile.source_id())
            {
                ...
                for dy in 0..footprint_height {
                    for dx in 0..footprint_width {
                        ...
                        source_land_target_roots[idx] =
                            Some((i32::from(tile.x), i32::from(tile.y)));
                        let raw_origin =
    ...
    /// answers true. Mirrors `Island::active_fertilities` but on the
    /// simulation-side map so callers don't need to keep the original
    /// `Island` around.
    ...
    pub fn source_land_idle_anchor(
        ...
        match descriptor.kind() {
            ...
            0x34 if descriptor.bytes()[1] == self.island_id => self
                ...
                .and_then(|root| self.local_to_source_world(root)),
    ...
    /// `FUN_004451a0`'s undoubled target rectangle for the generic
    /// category-one through -three route builder. Static descriptors retain
    /// their addressed cell, while the stored `FUN_00463880` dimensions are
    /// converted from the doubled map grid back to this figure grid.
    pub fn source_ship_target_rect(
        &self,
        descriptor: SourceTargetDescriptor,
    ) -> Option<SourcePathTargetRect> {
        if !matches!(descriptor.kind(), 0x32 | 0x33 | 0x34)
            || descriptor.bytes()[1] != self.island_id
        {
            return None;
        }
        let local = (
        ...
        SourcePathTargetRect::new(
            (
                self.source_world_origin.0.div_euclid(2) + local.0,
                self.source_world_origin.1.div_euclid(2) + local.1,
            ),
            usize::from((raw_width / 2).max(1)),
            usize::from((raw_height / 2).max(1)),
        )
    }

    /// `FUN_004670d0` for a kind-`0x34` controller target. The source maps
    /// the descriptor cell through `FUN_00463900`, applies the oriented
    /// footprint dimensions, then accepts the radius-five raster generated by
    /// `FUN_00404d70` during startup.
    pub fn source_controller_city_target_reached(
        &self,
        descriptor: SourceTargetDescriptor,
        figure_position: (i32, i32),
    ) -> bool {
        if descriptor.kind() != 0x34 || descriptor.bytes()[1] != self.island_id {
            return false;
        }
        let local = (
            i32::from(descriptor.bytes()[2]),
        ...
        let Some(root) = self.source_static_target_root(local) else {
            return false;
        };
        let Some(index) = self.local_index(local) else {
            return false;
        };
        let Some((raw_width, raw_height)) =
            self.source_land_target_sizes.get(index).copied().flatten()
        else {
            return false;
        };
        ...
        let origin = (
            self.source_world_origin.0.div_euclid(2) + root.0,
            self.source_world_origin.1.div_euclid(2) + root.1,
        );
        let vertical =
            source_controller_city_target_axis_distance(figure_position.1, origin.1, height);
        if vertical > 5 {
            return false;
        }
        let horizontal =
            source_controller_city_target_axis_distance(figure_position.0, origin.0, width);
        const RADIUS_FIVE_RASTER: [i32; 6] = [5, 5, 5, 4, 3, 2];
        horizontal <= RADIUS_FIVE_RASTER[vertical as usize]
    }

    fn source_static_target_root(&self, local: (i32, i32)) -> Option<(i32, i32)> {
        self.source_land_target_roots
            .get(self.local_index(local)?)
            .copied()
            .flatten()
    }
    ...
    pub fn new_open(island_id: u8, width: u16, height: u16) -> Self {
        ...
        Self {
            ...
            source_land_target_roots: (0..i32::from(height))
                .flat_map(|y| (0..i32::from(width)).map(move |x| Some((x, y))))
                .collect(),
...
fn source_controller_city_target_axis_distance(position: i32, origin: i32, length: i32) -> i32 {
    let center_offset = (length - 1) / 2;
    let offset = position - (origin + center_offset);
    if offset < 1 {
        -offset
    } else {
...
mod tests {
    ...
    fn source_land_target_rect_matches_fun_00444fe0_static_and_packed_forms() {
        ...
        assert_eq!(
            map.source_ship_target_rect(SourceTargetDescriptor::from_source_kind34_island_cell(
                10, 1, 2
            )),
            SourcePathTargetRect::new((111, 132), 4, 2)
        );
        assert_eq!(
    ...
    #[test]
    fn source_controller_city_target_reached_uses_fun_004670d0_radius_five_raster() {
        let mut map = IslandMap::new_open(10, 16, 16);
        map.source_world_origin = (220, 260);
        let target = SourceTargetDescriptor::from_source_kind34_island_cell(10, 3, 4);

        assert!(map.source_controller_city_target_reached(target, (113, 134)));
        assert!(map.source_controller_city_target_reached(target, (118, 134)));
        assert!(!map.source_controller_city_target_reached(target, (119, 134)));
        assert!(map.source_controller_city_target_reached(target, (117, 137)));
        assert!(!map.source_controller_city_target_reached(target, (118, 137)));
    }
