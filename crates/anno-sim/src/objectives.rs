...
//! revert. The game binary draws them in a panel and drains completion
//! indices for its notification cue.
    ...
    pub fn label(&self) -> String {
        match self {
            ...
            Objective::Monopolies { goods } => {
                let names: Vec<String> = goods.iter().map(|g| format!("{g:?}")).collect();
                format!("Monopoly: {}", names.join(" + "))
            }
            Objective::SupportFellowPlayer {
                who,
                target_population,
            } => {
...
impl ObjectiveSet {
    ...
    pub fn evaluate(
        ...
        for (idx, (obj, done)) in self.items.iter_mut().enumerate() {
            ...
            let met = match obj {
                ...
                Objective::ReachTotalPopulation { count } => player.total_population() >= *count,
                Objective::Build { prod_kind, count } => {
                    ...
                    let n = buildings
                        ...
                        .filter(|b| {
                            b.owner == owner
                                ...
                                && defs[b.def_id as usize].prod_kind.to_ascii_uppercase() == needle
                        })
                        .count();
                    n as u32 >= *count
                }
    ...
    /// goals.
    pub fn from_mission_goals(goals: &anno_formats::szs::MissionGoals) -> Self {
        let mut items = Vec::new();
        for slot in [goals.primary, goals.secondary, goals.tertiary] {
            if let Some(p) = slot {
                items.push(Objective::ReachTotalPopulation { count: p.total });
...
mod tests {
    ...
    #[test]
    fn default_objective_set_is_empty() {
    ...
    fn total_population_objective_completes_on_sum() {
        ...
        let mut set = ObjectiveSet::new(vec![Objective::ReachTotalPopulation { count: 5_000 }]);
    ...
    fn population_objective_completes() {
        let mut set = ObjectiveSet::new(vec![Objective::ReachPopulation {
            tier: 1,
            count: 100,
        }]);
        let mut p = Player::new_human(0);
    ...
    fn monopoly_completes_when_only_owner_produces() {
        ...
        let mut set = ObjectiveSet::new(vec![Objective::Monopolies {
        ...
        let mut set2 = ObjectiveSet::new(vec![Objective::Monopolies {
            goods: vec![Good::Tools],
        }]);
    ...
    fn support_fellow_player_completes_when_target_pop_reached() {
        let mut set = ObjectiveSet::new(vec![Objective::SupportFellowPlayer {
            who: 1,
    ...
    fn gold_objective_does_not_revert() {
        let mut set = ObjectiveSet::new(vec![Objective::AccumulateGold { amount: 1000 }]);
        let mut p = Player::new_human(0);
        p.gold = 1500;
